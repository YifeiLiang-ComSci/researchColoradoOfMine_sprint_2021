...
def apply(image, model=None, force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
    ...
    # if len(inimg_raw.shape) == 2:
    #     inimg_raw = inimg_raw.reshape((1, inimg_raw.shape[0], inimg_raw.shape[1]))
    # elif len(inimg_raw.shape) == 3:
    #     noHU = True ## noHU should be always True if input is color image and has RGB channel.
    #     pass
    #     # inimg_raw = inimg_raw.reshape((1, inimg_raw.shape[0], inimg_raw.shape[1], inimg_raw.shape[2]))
    #     # inimg_raw = skimage.color.rgb2gray(inimg_raw)
    #     # inimg_raw = inimg_raw.reshape((1, inimg_raw.shape[0], inimg_raw.shape[1]))
    # else:
    #     raise Exception(f"Unsupported dimension: {inimg_raw.shape}")
    ...
    if not noHU:
        ...
        # support for non HU images, or 3D image with RGB channel. This is just a hack. The models were not trained with this in mind
    ...
    result = outmask.astype(np.uint8)
    num_elements = 1
    for dim in result.shape:
        num_elements *= dim
    mask_density = np.count_nonzero(result) / num_elements
    if mask_density < 0.05:
        return f"Too small mask density: {mask_density}"
    else:
        return result
